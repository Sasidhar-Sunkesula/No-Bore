...
const WatchVideo = () => {
  ...
  const videoList = useSelector((store: any) => store.videoList.value);
  const [isLoading, setIsLoading] = useState(true);
  const watchListFromStore = useSelector((store: any) => store.watchLater.value);
  const subscriptionsList = useSelector((store: any) => store.subscriptions.value);
  const navigate = useNavigate();
  const dispatchFun = useDispatch();
  const [index, setIndex] = useState<number>(-1);
  const [subscribedIndex, setSubscribedIndex] = useState<number>(-1);
  ...
  useEffect(() => {
    closeSideBar();
    const selectedVideo = videoList.find((item: any) => item.id === reqId);
    if (selectedVideo) {
      setVideo(selectedVideo);
      const indexFromStore = watchListFromStore.findIndex(
        (obj: any) => obj.id === selectedVideo.id
      );
      setIndex(indexFromStore);
      const channelIndexFromStore = subscriptionsList.findIndex(
        (channelName: string) => channelName === selectedVideo.snippet.channelTitle
      );
    ...
    <div className="m-4 dark:bg-gray-800 dark:text-white text-black w-full">
      ...
      <div className="w-2/3">
        ...
        <div className="flex py-3 px-4 items-center justify-between">
          ...
          <div>
            <button onClick={addToWatchlist} className="btn mr-4">
