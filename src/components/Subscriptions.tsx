...
const Subscriptions: React.FC<SubscriptionsProps> = () => {
  ...
  const subscriptionsList = useSelector(
    (store: any) => store.subscriptions.value
  );
  const user = JSON.parse(localStorage.getItem("user")!);

  const remove = async (item: string) => {
    const index = subscriptionsList.findIndex((channel: string) => channel === item);
    dispatchFun(removeFromSubscriptionsList(index));

    await fetch(
      "https://nobore-backend.onrender.com/api/subscriptions/removeFromList",
      {
        method: "DELETE",
        headers: {
  ...
  return (
    <div className="w-full ">
      ...
      <div className="h-fit w-4/5 mx-auto grid grid-cols-1 gap-x-10 gap-y-5 md:grid-cols-2 p-7 my-1">
        {subscriptionsList.map((channel: string) => (
          <div
            key={channel}
            className="flex-shrink-0  border rounded-lg shadow-sm h-fit py-5 pl-5 pr-10 group block"
          >
